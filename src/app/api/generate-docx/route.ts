...
import { createReadStream, readFileSync } from 'fs'; // Importing file system functions for reading files and creating streams
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const params = new DocumentMergeParams({
      jsonDataForMerge,
      outputFormat: OutputFormat.DOCX, // Specifying the output format as DOCX
    });
    ...
    const filename = `mergedOutput_${timestamp}.docx`; // Constructing the filename

    // Returning the merged DOCX file as a response
